...
//		Log.d(TAG, name + " ispatchDraw " + mState);
	...
	OnTouchListener touchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			Log.e("onTouch", "state: " + mState + " x: " + event.getX() + " y: " + event.getY());
			int action = event.getAction();
			if (action == MotionEvent.ACTION_DOWN) {
				initX = 0;
				initY = 0;
				if (mContent.getVisibility() == GONE) {
                    Log.e("onTouch 2", "down");
...
//					// ... and set initial panel's position
                ...
                Log.e("onTouch3", "action: " + action + " initX, initY:" + initX + " " + initY);
		...
		public void run() {
			...
			if (mOrientation == VERTICAL) {
				...
				if (!mIsShrinking) {
					...
					toXDelta = mPosition == LEFT? -width : (width + 20);
				}
				if (mState == State.TRACKING) {
    ...
    public PanelOnGestureListener getmGestureListener() {
        return mGestureListener;
    }

	public class PanelOnGestureListener implements OnGestureListener {
		float scrollY;
		float scrollX;
        boolean isXinited = false;
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			...
			float tmpY = 0, tmpX = 0;
            Log.e("onScroll", "mPositoin: scrollX, distanceX" + mPosition + " " + scrollX  + " " + distanceX + " scrollY, distanceY:  " + scrollY + " " + distanceY);
            if(!isXinited) {
                distanceX += 1440;
                isXinited = true;
            }
            if (mOrientation == VERTICAL) {
				...
				if (mPosition == LEFT) {
					...
					tmpX = ensureRange(scrollX, -mContentWidth, mContentWidth);
				}
			}
